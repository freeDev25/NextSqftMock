    ...
    public function show_field_repeater($field, $meta)
    {
      ...
      if (count($meta) > 0 && is_array($meta)) {
        foreach ($meta as $me) {
          ...
          echo '
        <span class="re-control at-re-toggle"><img src="' . $plugin_path . '/images/edit.png" alt="Edit" class="edit-mmb" title="Edit"/></span> 
        <img src="' . $plugin_path . '/images/remove.png" alt="' . __('Remove', 'mmb') . '" title="' . __('Remove', 'mmb') . '" class="remove-mmb" id="remove-' . $field['id'] . '">
        <span class="re-control-clear"></span></div>';
          $c = $c + 1;
        }
      }
      //echo "<span class='desc'>".__('Click Here to add repeatable field.','mmb')."</span>";
      echo '<img src="';
      ...
      echo '" alt="' . __('Add', 'mmb') . '" title="' . __('Add', 'mmb') . '" class="add-mmb"  id="add-' . $field['id'] . '"><br/></div>';
      ...
      echo '</table><img src="' . $plugin_path . '/images/remove.png" alt="' . __('Remove', 'mmb') . '" class="remove-mmb" title="' . __('Remove', 'mmb') . '" id="remove-' . $field['id'] . '"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px;
    ...
    {
      echo "<td class='at-field clearfix'" . (($this->inGroup === true) ? " valign='top'" : "") . ">";
      if ($field['name'] != '' || $field['name'] != FALSE) {
        echo "<div class='components-base-control__label'>";
    ...
    {
      $this->show_field_begin($field, $meta);
      echo "<input type='text' class='at-text" . (isset($field['class']) ? ' ' . $field['class'] : '') . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' " . (isset($field['style']) ? "style='{$field['style']}'" : '') . "/><div class='clearfix'></div>";
      $this->show_field_end($field, $meta);
    }
