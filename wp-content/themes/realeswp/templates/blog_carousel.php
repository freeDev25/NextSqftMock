...
while( $posts->have_posts() ) {
    ...
    if($post_image === false) {
        //$post_image = array(get_template_directory_uri() . '/images/default-image.jpg');
    }
    ...
    $display .= '<div class="carousel-title">' . __('Featured on Blog', 'realeswp') . '</div>';
    $display .= '<div class="caption-title">' . esc_html($post_title) . '</div>';
    $display .= '<div class="caption-subtitle">' . esc_html($post_excerpt) . '</div>';
    $display .= '<a href="' . esc_url(get_permalink($post_id)) . '" class="btn btn-lg btn-o btn-white">' . __('Read More', 'realeswp') . '</a>';
    $display .= '</div>';
