...
<div class="similar">
    <h3><?php esc_html_e('Properties Listed by Sellers', 'realeswp'); ?></h3>
    <?php
    $orig_post = $post;
        ...
        <?php while( $my_query->have_posts() ) {
            ...
            if(isset($s_images[1])) {
                if(function_exists('aq_resize')) {
                    $s_thumb = aq_resize($s_images[1], 600, 400, true);
                    if($s_thumb == false) {
                        $s_thumb = $s_images[1];
                    }
                } else {
                ...
                $s_thumb = get_template_directory_uri() . '/images/default-image-600x400.jpg';
            }
        ?>
        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
            <a href="<?php echo esc_url($s_link); ?>" class="similarProp card-min" data-prop="<?php echo esc_attr($s_id); ?>">
                ...
                <div class="info text-nowrap">
                    ...
                    <div class="price">
                        <?php if($s_currency_pos == 'before') {
                            echo esc_html($s_currency) . esc_html($s_price) . ' ' . esc_html($s_price_label);
                        } else {
                            echo esc_html($s_price) . esc_html($s_currency) . ' ' . esc_html($s_price_label);
                        }

                        if($s_type) {
                            $s_type_color = get_term_meta($s_type[0]->term_id, 'property_type_category_color', true);
                            ...
                            if($s_type_color == '') {
                                ...
                                $p_type_color = $s_type_color;
                            } ?>

                            <span class="badge" style="background-color: <?php echo esc_attr($p_type_color); ?>"><?php echo esc_html($s_type[0]->name); ?></span>
                        <?php } ?>
                    </div>
                </div>
    ...
    <?php } else {
        print '<div class="noProp">' . __('No listed properties found.', 'realeswp') . '</div>';
    }
